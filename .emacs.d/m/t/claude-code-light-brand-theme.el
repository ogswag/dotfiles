;;; claude-code-light-brand-theme.el --- Claude Code Light Brand -*- no-byte-compile: t; lexical-binding: t; -*-
...
;; A single-file Emacs port of "Claude Code Light Brand" from the VSCode theme
;; collection ashwingopalsamy/claude-code-theme.  Identical to
;; `claude-code-light' but with Anthropic's saturated terracotta brand accent
...
;;;###theme-autoload
(deftheme claude-code-light-brand
  "Light Claude Code theme with the saturated terracotta brand accent.
Ported from the Claude Code Light Brand VSCode theme."
...
(let* ((class '((class color) (min-colors 89)))
       ...
       (bg        "#faf9f5")
       (bg-alt    "#f3f1e9")
       (bg-hl     "#f6ede7") ; current-line tint (brand accent over bg)
       (fg        "#1a1917")
       (ui-border "#d9d5cc")
       (line-num  "#8d877d")
       (muted     "#6b665f")
       ...
       (punct     "#877c70")
       (op        "#6a645c")
       (sel       "#f1ded5") ; opaque tint of brand selectionBackground
       (lazy-bg   "#f3e0d2")
...
(provide-theme 'claude-code-light-brand)

(provide 'claude-code-light-brand-theme)

;;; claude-code-light-brand-theme.el ends here
