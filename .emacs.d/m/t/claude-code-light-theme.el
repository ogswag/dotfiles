;;; claude-code-light-theme.el --- Claude Code Light -*- no-byte-compile: t; lexical-binding: t; -*-
...
;; A single-file Emacs port of "Claude Code Light" from the VSCode theme
;; collection ashwingopalsamy/claude-code-theme.  Anthropic's warm "Claude"
;; brand palette: a terracotta accent over paper neutrals.
;;
;; All colors are collected in the `let*' palette below; edit a color there
;; once and every face that uses it updates.

;;; Code:

;;;###theme-autoload
(deftheme claude-code-light
  "Light, warm Claude Code theme (terracotta accent on paper neutrals).
Ported from the Claude Code Light VSCode theme."
...
(let* ((class '((class color) (min-colors 89)))
       ...
       (bg        "#faf9f5") ; paper / editor background
       (bg-alt    "#f3f1e9") ; surface: sidebar, panels, inputs
       (bg-hl     "#f6efe9") ; current-line (lineHighlight tint over bg)
       (fg        "#1a1917") ; ink / main foreground
       (ui-border "#d9d5cc") ; borders, indent guides
       (line-num  "#8d877d") ; line numbers / subtle
       (muted     "#6b665f") ; dim text
       (comment   "#6c655d") ; comments
       (punct     "#877c70") ; punctuation
       (op        "#6a645c") ; operators
       (sel       "#f2e2da") ; opaque tint of selectionBackground
       (lazy-bg   "#f3e4d6") ; warm tint for lazy-highlight
       ...
       ;; Syntax accents
       (kw        "#b84a2a") ; keyword / storage
       (str       "#2d7f4d") ; string
       (num       "#946a1e") ; number / constant
       (param     "#ae6d53") ; parameter
       (func      "#ae4e30") ; function
       (type      "#386290") ; type / class
       (tag       "#cc5e54") ; tag
       (attr      "#b46344") ; attribute
...
(provide-theme 'claude-code-light)

(provide 'claude-code-light-theme)

;;; claude-code-light-theme.el ends here
