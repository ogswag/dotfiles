;;; claude-code-dark-high-contrast-theme.el --- Claude Code Dark High Contrast -*- no-byte-compile: t; lexical-binding: t; -*-
...
;; A single-file Emacs port of "Claude Code Dark High Contrast" from the VSCode
;; theme collection ashwingopalsamy/claude-code-theme.  Anthropic's warm
;; "Claude" brand palette with a deeper background and stronger accent/borders.
;;
;; All colors are collected in the `let*' palette below; this variant differs
;; from `claude-code-dark' only in that block.

;;; Code:

;;;###theme-autoload
(deftheme claude-code-dark-high-contrast
  "High-contrast dark Claude Code theme (terracotta accent on deep ink).
Ported from the Claude Code Dark High Contrast VSCode theme."
...
(let* ((class '((class color) (min-colors 89)))
       ...
       (bg        "#11100f") ; editor background (deeper)
       (bg-alt    "#1b1916") ; surface
       (bg-hl     "#2a221e") ; current-line tint
       (fg        "#f0eee6") ; brighter foreground
       (ui-border "#6b665f") ; stronger borders
       (line-num  "#8d877d")
       (muted     "#bdb6aa")
       (comment   "#c4bbb0")
       (punct     "#cfc6bb")
...
(provide-theme 'claude-code-dark-high-contrast)

(provide 'claude-code-dark-high-contrast-theme)

;;; claude-code-dark-high-contrast-theme.el ends here
